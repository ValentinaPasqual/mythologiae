...
######################################################## FACTUAL DATA ###############################################################################################
            ...
            if row['riscritture_letterarie'] != {}:
                for el2 in row['riscritture_letterarie']: # el2 = dizionario
                    try:
                        ...
                        g.get_context(factual_data).add((URIRef(work + el2['work_uri']), dct.type, Literal('riscrittura letteraria', datatype=XSD.string)))
            ...
            if row['fonti_medievali_e_moderne'] != {}:
                for el2 in row['fonti_medievali_e_moderne']:  # el2 = dizionario
                    try:
                        ...
                        g.get_context(assertion).add((URIRef(work + el2['work_uri']), ecrm.P67_refers_to, (URIRef(categ + el['uri']))))
                        g.get_context(factual_data).add((URIRef(work + el2['work_uri']), rdfs.label, Literal(el2['work_label'], datatype=XSD.string)))
                        g.get_context(factual_data).add((URIRef(work + el2['work_uri']), dct.type, Literal('fonte medievale o moderna', datatype=XSD.string)))
                        # manca subwork_uri e subwork_label
                        # manca la data !!!!
                        for autore_uri in el2['autore_uri']:
                            g.get_context(factual_data).add((URIRef(people + autore_uri), RDF.type, efrbroo.F10_Person))
            ...
            if row['riscritture_cinematografiche'] != {}:
                for el3 in row['riscritture_cinematografiche']:
                    try:
                        ...
                        g.get_context(assertion).add((URIRef(work + el3['titolo_film_uri']), ecrm.P67_refers_to, (URIRef(categ + el['uri']))))
                        g.get_context(factual_data).add((URIRef(work + el3['titolo_film_uri']), rdfs.label, Literal(el3['titolo_film'], datatype=XSD.string)))
                        g.get_context(factual_data).add((URIRef(work + el3['titolo_film_uri']), dct.type,  Literal('riscrittura cinematografica', datatype=XSD.string)))
                            # manca la data !!!!
    ...
    #factual_data.serialize(destination='discarica/rdf_liv0_works.trig', format='trig')
        ...
        g.get_context(factual_data2).add((item + URIRef(row['item_id'] + '-creation'), efrbroo.R18_created, (item + URIRef(row['item_id']))))
        g.get_context(factual_data2).add((item + URIRef(row['item_id'] + '-creation'), efrbroo.R18_created, (item + URIRef(row['item_id'] + '-expression'))))
        ...
        if row.get('item_data') != None:
            if row.get('item_data') != {}:
                ...
                g.get_context(factual_data2).add((URIRef(item + row['item_id'] + '-creation'), ecrm.P10_falls_within,(URIRef(time + dy.get('anno_uri')))))
                g.get_context(factual_data2).add((URIRef(time + row.get('item_data').get('anno_uri')), RDF.type, ecrm.E52_Time_Span))
                # L'ONTO NON E' MAI ECRM!!!!!! MA 6.2.1 DI CIDOC
                g.get_context(factual_data2).add((URIRef(time + dy.get('anno_uri')), rdfs.label, Literal(dy.get('anno_label'), datatype=XSD.string)))
                g.get_context(factual_data2).add((URIRef(time + dy.get('anno_uri')), ecrm.P82a_begin_of_the_begin, Literal(dy.get('anno_start'), datatype=XSD.date)))
                g.get_context(factual_data2).add((URIRef(time + dy.get('anno_uri')), ecrm.P82b_end_of_the_end, Literal(dy.get('anno_end'), datatype=XSD.date)))
                g.get_context(factual_data2).add((URIRef(time + dy.get('anno_uri')), dct.type, Literal('data di concezione dell\' item', datatype=XSD.string)))
        ...
        if row.get('item_periodo') != None:
            ...
            g.get_context(factual_data2).add((time + URIRef(row.get('item_periodo')), dct.type, Literal('periodo di concezione dell\' item', datatype=XSD.string)))
            ...
            if row.get('item_secolo') != "":
                for diz in row.get('item_secolo'):
                    if diz != {}:
                        ...
                        g.get_context(factual_data2).add((time + URIRef(diz.get('item_secolo_uri')), dct.type, Literal('secolo di concezione dell\' item',datatype=XSD.string)))
        ...
        if row.get('item_tipologia') != None:
            ...
            g.get_context(factual_data2).add((item + URIRef(row['item_id']), ecrm.P2_has_type, (item + URIRef(row.get('item_tipologia')))))

        if row.get('item_descrizione') != None:
            if row.get('item_descrizione') != "":
                g.get_context(factual_data2).add((item + URIRef(row['item_id'] + '-expression'), dct.description, Literal(row.get('item_descrizione'), datatype=XSD.string)))

        if row.get('item_note') != None:
            if row.get('item_note') != "":
                g.get_context(factual_data2).add((item + URIRef(row['item_id']), dct.note, Literal(row.get('item_note'), datatype=XSD.string)))
...
######################################################## FACTUAL DATA E FONTI CLASSICHE ###############################################################################################
                    ...
                    if diz_tsv['author'] in result['author']:
                        if diz_tsv['work'] in result['work']:
                            if diz_cl['work'] == result['cts']:
                                ...
                                for cat in row['categoria']:
                                    g.get_context(assertion).add((URIRef(diz_cl['work']), ecrm.P67_refers_to, (URIRef(categ + cat['uri']))))
                                ...
                                g.get_context(factual_data).add((URIRef(diz_cl['work']), dct.type, Literal('fonte classica', datatype=XSD.string)))
                                g.get_context(factual_data).add((URIRef(diz_cl['title']), RDF.type, ecrm.E35_Title))
                                ...
                                g.get_context(factual_data).add((URIRef(diz_cl['person_urn_cts']), rdfs.label,  Literal(diz_cl['person_urn_cts_label'], datatype=XSD.string)))
    ...
    #g.serialize(destination='discarica/grafo_wip_12-09.trig', format='trig')
